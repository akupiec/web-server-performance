...
import fs from "fs";
import {convertToBytes, rawStatsToBytesHuman, rawStatsToPercentages} from "./utils/human.js";
...
function calculateStats(values) {
    ...
    const avg = parseFloat((values.reduce((sum, value) => sum + value, 0) / n).toFixed(2));
    ...
    function getPercentile(percentile) {
        ...
        return parseFloat((values[lower] * (upper - index) + values[upper] * (index - lower)).toFixed(2));
    }
